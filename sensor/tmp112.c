...
static void i2c_transfer(uint32_t i2c, uint8_t addr, uint8_t* w, size_t wn,
                         uint8_t* r, size_t rn);
...
void tmp112_end(void) {
    ...
    uint8_t config[3] = {TMP112_SEL_CONFIG_REG, TMP112_CONFIG_OS_MSB,
                         TMP112_CONFIG_OS_LSB};
    // uint8_t config[3] = {TMP112_SEL_CONFIG_REG, TMP112_CONFIG_CM_MSB,
    // TMP112_CONFIG_CM_LSB};
...
static void tmp112_i2c_setup(void) {
    ...
    gpio_mode_setup(TEMP_I2C_SCL_PORT, GPIO_MODE_AF, GPIO_PUPD_PULLUP,
                    TEMP_I2C_SCL);
    gpio_mode_setup(TEMP_I2C_SDA_PORT, GPIO_MODE_AF, GPIO_PUPD_PULLUP,
                    TEMP_I2C_SDA);

    gpio_set_output_options(TEMP_I2C_SCL_PORT, GPIO_OTYPE_OD, GPIO_OSPEED_2MHZ,
                            TEMP_I2C_SCL);
    gpio_set_output_options(TEMP_I2C_SDA_PORT, GPIO_OTYPE_OD, GPIO_OSPEED_2MHZ,
                            TEMP_I2C_SDA);
    ...
    // i2c_set_speed(TEMP_I2C, i2c_speed_sm_100k,
...
static void i2c_transfer(uint32_t i2c, uint8_t addr, uint8_t* w, size_t wn,
    ...
    if (wn) {
        ...
        while (wn--) {
            ...
            TIMEOUT(100000, "TMP I2C:", ((wn << 16) | *w),
