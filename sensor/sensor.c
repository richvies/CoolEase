...
void set_gpio_for_standby(void) {
    ...
    // TMP
    gpio_mode_setup(TEMP_I2C_SCL_PORT, GPIO_MODE_INPUT, GPIO_PUPD_PULLUP,
                    TEMP_I2C_SCL);
...
static void test(void) {
    ...
    // serial_printf("Dev Info Location: %8x %8x %8x\n", EEPROM_APP_INFO_BASE,
    // &app_info->aes_key[0], app_info->aes_key);
...
static void send_packet(void) {
    ...
    rfm_config_for_lora(RFM_BW_125KHZ, RFM_CODING_RATE_4_5,
                        RFM_SPREADING_FACTOR_128CPS, true, rf_power);
