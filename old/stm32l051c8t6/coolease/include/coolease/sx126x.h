...
#define SX126X_SLEEP_START_COLD                       0           //  2     2     sleep mode: cold start, configuration is lost (default)
#define SX126X_SLEEP_START_WARM                       1 << 2      //  2     2                 warm start, configuration is retained
#define SX126X_SLEEP_RTC_OFF                          0           //  0     0     wake on RTC timeout: disabled
#define SX126X_SLEEP_RTC_ON                           1 << 0      //  0     0                          enabled
...
//SX126X_CMD_SET_RX
#define SX126X_RX_TIMEOUT_NONE                        (uint32_t)0x000000    //  23    0     Rx timeout duration: no timeout (Rx single mode)
#define SX126X_RX_TIMEOUT_INF                         (uint32_t)0xFFFFFF    //  23    0                          infinite (Rx continuous mode)
...
#define SX126X_CALIBRATE_IMAGE_OFF                    0           //  6     6     image calibration: disabled
#define SX126X_CALIBRATE_IMAGE_ON                     (1 << 6)      //  6     6                        enabled
#define SX126X_CALIBRATE_ADC_BULK_P_OFF               0           //  5     5     ADC bulk P calibration: disabled
#define SX126X_CALIBRATE_ADC_BULK_P_ON                (1 << 5)      //  5     5                             enabled
#define SX126X_CALIBRATE_ADC_BULK_N_OFF               0           //  4     4     ADC bulk N calibration: disabled
#define SX126X_CALIBRATE_ADC_BULK_N_ON                (1 << 4)      //  4     4                             enabled
#define SX126X_CALIBRATE_ADC_PULSE_OFF                0           //  3     3     ADC pulse calibration: disabled
#define SX126X_CALIBRATE_ADC_PULSE_ON                 (1 << 3)      //  3     3                            enabled
#define SX126X_CALIBRATE_PLL_OFF                      0           //  2     2     PLL calibration: disabled
#define SX126X_CALIBRATE_PLL_ON                       (1 << 2)      //  2     2                      enabled
#define SX126X_CALIBRATE_RC13M_OFF                    0           //  1     1     13 MHz RC osc. calibration: disabled
#define SX126X_CALIBRATE_RC13M_ON                     (1 << 1)      //  1     1                                 enabled
#define SX126X_CALIBRATE_RC64K_OFF                    0           //  0     0     64 kHz RC osc. calibration: disabled
#define SX126X_CALIBRATE_RC64K_ON                     (1 << 0)      //  0     0                                 enabled
...
#define SX126X_IRQ_TIMEOUT                          (1 << 9)        //  9     9     Rx or Tx timeout
#define SX126X_IRQ_CAD_DETECTED                     (1 << 8)        //  8     8     channel activity detected
#define SX126X_IRQ_CAD_DONE                         (1 << 7)        //  7     7     channel activity detection finished
#define SX126X_IRQ_CRC_ERR                          (1 << 6)        //  6     6     wrong CRC received
#define SX126X_IRQ_HEADER_ERR                       (1 << 5)        //  5     5     LoRa header CRC error
#define SX126X_IRQ_HEADER_VALID                     (1 << 4)        //  4     4     valid LoRa header received
#define SX126X_IRQ_SYNC_WORD_VALID                  (1 << 3)        //  3     3     valid sync word detected
#define SX126X_IRQ_PREAMBLE_DETECTED                (1 << 2)        //  2     2     preamble detected
#define SX126X_IRQ_RX_DONE                          (1 << 1)        //  1     1     packet received
#define SX126X_IRQ_TX_DONE                          (1 << 0)        //  0     0     packet transmission completed
#define SX126X_IRQ_ALL                              0x3FF         //  9     0     all interrupts
#define SX126X_IRQ_NONE                             0x000         //  9     0     no interrupts
...
#define SX126X_STATUS_MODE_STDBY_RC                   (1 << 5)      //  6     4     current chip mode: STDBY_RC
#define SX126X_STATUS_MODE_STDBY_XOSC                 48            //  6     4                        STDBY_XOSC
#define SX126X_STATUS_MODE_FS                         64            //  6     4                        FS
#define SX126X_STATUS_MODE_RX                         80            //  6     4                        RX
#define SX126X_STATUS_MODE_TX                         96            //  6     4                        TX
#define SX126X_STATUS_DATA_AVAILABLE                  4             //  3     1     command status: packet received and data can be retrieved
#define SX126X_STATUS_CMD_TIMEOUT                     6             //  3     1                     SPI command timed out
#define SX126X_STATUS_CMD_INVALID                     8             //  3     1                     invalid SPI command
#define SX126X_STATUS_CMD_FAILED                      10            //  3     1                     SPI command failed to execute
#define SX126X_STATUS_TX_DONE                         12            //  3     1                     packet transmission done

//SX126X_CMD_GET_PACKET_STATUS
#define SX126X_GFSK_RX_STATUS_PREAMBLE_ERR            (1 << 7)  //  7     7     GFSK Rx status: preamble error
#define SX126X_GFSK_RX_STATUS_SYNC_ERR                (1 << 6)  //  6     6                     sync word error
#define SX126X_GFSK_RX_STATUS_ADRS_ERR                (1 << 5)  //  5     5                     address error
#define SX126X_GFSK_RX_STATUS_CRC_ERR                 (1 << 4)  //  4     4                     CRC error
#define SX126X_GFSK_RX_STATUS_LENGTH_ERR              (1 << 3)  //  3     3                     length error
#define SX126X_GFSK_RX_STATUS_ABORT_ERR               (1 << 2)  //  2     2                     abort error
#define SX126X_GFSK_RX_STATUS_PACKET_RECEIVED         (1 << 1)  //  2     2                     packet received
#define SX126X_GFSK_RX_STATUS_PACKET_SENT             (1 << 0)  //  2     2                     packet sent

//SX126X_CMD_GET_DEVICE_ERRORS
#define SX126X_PA_RAMP_ERR                           (1 << 7)  //  8     8     device errors: PA ramping failed
#define SX126X_PLL_LOCK_ERR                          (1 << 6)  //  6     6                    PLL failed to lock
#define SX126X_XOSC_START_ERR                        (1 << 5)  //  5     5                    crystal oscillator failed to start
#define SX126X_IMG_CALIB_ERR                         (1 << 4)  //  4     4                    image calibration failed
#define SX126X_ADC_CALIB_ERR                         (1 << 3)  //  3     3                    ADC calibration failed
#define SX126X_PLL_CALIB_ERR                         (1 << 2)  //  2     2                    PLL calibration failed
#define SX126X_RC13M_CALIB_ERR                       (1 << 1)  //  1     1                    RC13M calibration failed
#define SX126X_RC64K_CALIB_ERR                       (1 << 0)  //  0     0                    RC64K calibration failed
...
/*
 * SX126X data packet structure
 */
typedef struct
{
    enum
    {
        SX126X_PACKET_DEV_NUM_0 = 0, SX126X_PACKET_DEV_NUM_1, SX126X_PACKET_DEV_NUM_2, SX126X_PACKET_DEV_NUM_3,
        SX126X_PACKET_POWER, SX126X_PACKET_BATTERY, 
        SX126X_PACKET_MSG_NUM_0, SX126X_PACKET_MSG_NUM_1, SX126X_PACKET_MSG_NUM_2, SX126X_PACKET_MSG_NUM_3, 
        SX126X_PACKET_TEMP_0, SX126X_PACKET_TEMP_1
    }data_enum;

    #define SX126X_PACKET_LENGTH 16

    // Data Buffer
    uint8_t data[SX126X_PACKET_LENGTH];

    bool crc_ok;
    bool header_ok;

    // Usefull info. RSSI etc.
    int8_t snr;
    int16_t rssi;
...
void        sx126x_start_listening(void);
