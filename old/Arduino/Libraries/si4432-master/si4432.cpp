...
Si4432::Si4432(uint8_t sdnPin, uint8_t InterruptPin, uint8_t ssPin) :
		_sdnPin(sdnPin), _intPin(InterruptPin), _ssPin(ssPin), _freqCarrier(433), _freqChannel(0), _kbps(100), _packageSign(
...
void Si4432::init() {
	...
	//digitalWrite(_ssPin, HIGH); // set pin high, so chip would know we don't use it. - well, it's turned off anyway but...
...
bool Si4432::waitForPacket(uint64_t waitMs) {
	...
	while (millis() - enterMillis < waitMs) {
		Serial.print(ReadRegister(REG_RSSI)); Serial.print(" ");
		if (!isPacketReceived()) {
			continue;
		} else {
			return true;
