...
#include "coolease/serial_spf_serial_printf.h"
...
bool w25_Init(void)
{
	...
	spf_serial_printf("w25 Page Count: %d\r\n",w25.PageCount);
...
uint32_t 	w25_ReadID(void)
{
	...
	spi_xfer(0x9F);
	Temp0 = spi_xfer(W25_DUMMY_BYTE);
	Temp1 = spi_xfer(W25_DUMMY_BYTE);
	Temp2 = spi_xfer(W25_DUMMY_BYTE);
...
void 		w25_ReadUniqID(void)
{
	...
	for(uint8_t	i=0;i<4;i++)
		spi_xfer(W25_DUMMY_BYTE);

	for(uint8_t	i=0;i<8;i++)
		w25.UniqID[i] = spi_xfer(W25_DUMMY_BYTE);
...
void w25_WaitForWriteEnd(void)
{
	...
	do
	{
		w25.StatusRegister1 = spi_xfer(W25_DUMMY_BYTE);
...
uint8_t w25_ReadStatusRegister(uint8_t	SelectStatusRegister_1_2_3)
{
	...
	{
		spi_xfer(0x05);
		status=spi_xfer(W25_DUMMY_BYTE);	
...
void	w25_EraseChip(void)
{
	...
	#if (_W25_DEBUG==1)
	spf_serial_printf("w25 EraseBlock done after %d ms!\r\n",HAL_GetTick()-StartTime);
	#endif
...
void 	w25_EraseSector(uint32_t SectorAddr)
{
  ...
  spi_xfer(0x20);
	if(w25.ID>=W25Q256)
		spi_xfer((SectorAddr & 0xFF000000) >> 24);
  spi_xfer((SectorAddr & 0xFF0000) >> 16);
  spi_xfer((SectorAddr & 0xFF00) >> 8);
  spi_xfer(SectorAddr & 0xFF);
	  	spi_chip_deselect(); 
  	timers_delay_microseconds(1);
  w25_WaitForWriteEnd();
	#if (_W25_DEBUG==1)
	spf_serial_printf("w25 EraseSector done after %d ms\r\n",HAL_GetTick()-StartTime);
	#endif
	timers_delay_microseconds(1);
	w25.Lock=0;
}
void 	w25_EraseBlock(uint32_t BlockAddr)
{
  ...
  spi_xfer(0xD8);
	if(w25.ID>=W25Q256)
		spi_xfer((BlockAddr & 0xFF000000) >> 24);
  spi_xfer((BlockAddr & 0xFF0000) >> 16);
  spi_xfer((BlockAddr & 0xFF00) >> 8);
  spi_xfer(BlockAddr & 0xFF);
	  	spi_chip_deselect(); 
  	timers_delay_microseconds(1);
  w25_WaitForWriteEnd();
	#if (_W25_DEBUG==1)
...
bool 	w25_IsEmptyPage(uint32_t Page_Address,uint32_t OffsetInByte,uint32_t NumByteToCheck_up_to_PageSize)
{
	...
	for(i=OffsetInByte; i<w25.PageSize; i+=sizeof(pBuffer))
	{
  	...
  	timers_delay_microseconds(1);
		...
		spi_xfer(0x0B);
		if(w25.ID>=W25Q256)
			spi_xfer((WorkAddress & 0xFF000000) >> 24);
		spi_xfer((WorkAddress & 0xFF0000) >> 16);
		spi_xfer((WorkAddress & 0xFF00) >> 8);
		spi_xfer(WorkAddress & 0xFF);
		spi_xfer(0);
	...
	#if (_W25_DEBUG==1)
	spf_serial_printf("w25 CheckPage is Empty in %d ms\r\n",HAL_GetTick()-StartTime);
	timers_delay_microseconds(100);
	#endif	
	w25.Lock=0;
	return true;	
	NOT_EMPTY:
	#if (_W25_DEBUG==1)
	spf_serial_printf("w25 CheckPage is Not Empty in %d ms\r\n",HAL_GetTick()-StartTime);
	timers_delay_microseconds(100);
	#endif	
	w25.Lock=0;
	return false;
...
bool 	w25_IsEmptySector(uint32_t Sector_Address,uint32_t OffsetInByte,uint32_t NumByteToCheck_up_to_SectorSize)
{
	...
	#if (_W25_DEBUG==1)
	spf_serial_printf("w25 CheckSector is Empty in %d ms\r\n",HAL_GetTick()-StartTime);
	timers_delay_microseconds(100);
	#endif	
	w25.Lock=0;
	return true;	
	NOT_EMPTY:
	#if (_W25_DEBUG==1)
	spf_serial_printf("w25 CheckSector is Not Empty in %d ms\r\n",HAL_GetTick()-StartTime);
	timers_delay_microseconds(100);
	#endif	
	w25.Lock=0;
	return false;
...
bool 	w25_IsEmptyBlock(uint32_t Block_Address,uint32_t OffsetInByte,uint32_t NumByteToCheck_up_to_BlockSize)
{
	...
	#if (_W25_DEBUG==1)
	spf_serial_printf("w25 CheckBlock is Empty in %d ms\r\n",HAL_GetTick()-StartTime);
	timers_delay_microseconds(100);
	#endif	
	w25.Lock=0;
	return true;	
	NOT_EMPTY:
	#if (_W25_DEBUG==1)
	spf_serial_printf("w25 CheckBlock is Not Empty in %d ms\r\n",HAL_GetTick()-StartTime);
	timers_delay_microseconds(100);
	#endif	
	w25.Lock=0;
	return false;
...
void 	w25_WriteByte(uint8_t pBuffer, uint32_t WriteAddr_inBytes)
{
  ...
  spi_xfer(0x02);
	if(w25.ID>=W25Q256)
		spi_xfer((WriteAddr_inBytes & 0xFF000000) >> 24);
  spi_xfer((WriteAddr_inBytes & 0xFF0000) >> 16);
  spi_xfer((WriteAddr_inBytes & 0xFF00) >> 8);
  spi_xfer(WriteAddr_inBytes & 0xFF);
  spi_xfer(pBuffer);
	  	spi_chip_deselect(); 
  	timers_delay_microseconds(1);
  w25_WaitForWriteEnd();
	#if (_W25_DEBUG==1)
	spf_serial_printf("w25 WriteByte done after %d ms\r\n",HAL_GetTick()-StartTime);
	#endif
	w25.Lock=0;
}
void 	w25_WritePage(uint8_t *pBuffer	,uint32_t Page_Address,uint32_t OffsetInByte,uint32_t NumByteToWrite_up_to_PageSize)
{
  ...
  spi_xfer(0x02);
	...
	if(w25.ID>=W25Q256)
		spi_xfer((Page_Address & 0xFF000000) >> 24);
  spi_xfer((Page_Address & 0xFF0000) >> 16);
  spi_xfer((Page_Address & 0xFF00) >> 8);
  spi_xfer(Page_Address&0xFF);
  ...
  w25_WaitForWriteEnd();
	...
	spf_serial_printf("\r\n");
	spf_serial_printf("w25 WritePage done after %d ms\r\n",StartTime);
	timers_delay_microseconds(100);
	#endif	
	timers_delay_microseconds(1);
	w25.Lock=0;
...
void 	w25_ReadByte(uint8_t *pBuffer,uint32_t Bytes_Address)
{
  ...
  spi_xfer(0x0B);
	if(w25.ID>=W25Q256)
		spi_xfer((Bytes_Address & 0xFF000000) >> 24);
  spi_xfer((Bytes_Address & 0xFF0000) >> 16);
  spi_xfer((Bytes_Address& 0xFF00) >> 8);
  spi_xfer(Bytes_Address & 0xFF);
	spi_xfer(0);
	*pBuffer = spi_xfer(W25_DUMMY_BYTE);
	  	spi_chip_deselect(); 
  	timers_delay_microseconds(1);	
	#if (_W25_DEBUG==1)
	spf_serial_printf("w25 ReadByte 0x%02X done after %d ms\r\n",*pBuffer,HAL_GetTick()-StartTime);
	#endif
	w25.Lock=0;
}
void 	w25_ReadBytes(uint8_t* pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead)
{
	...
	spi_xfer(0x0B);
	if(w25.ID>=W25Q256)
		spi_xfer((ReadAddr & 0xFF000000) >> 24);
  spi_xfer((ReadAddr & 0xFF0000) >> 16);
  spi_xfer((ReadAddr& 0xFF00) >> 8);
  spi_xfer(ReadAddr & 0xFF);
	spi_xfer(0);
	...
	spf_serial_printf("\r\n");
	spf_serial_printf("w25 ReadBytes done after %d ms\r\n",StartTime);
	timers_delay_microseconds(100);
	#endif	
	timers_delay_microseconds(1);
	w25.Lock=0;
...
void 	w25_ReadPage(uint8_t *pBuffer,uint32_t Page_Address,uint32_t OffsetInByte,uint32_t NumByteToRead_up_to_PageSize)
{
  ...
  spi_xfer(Page_Address & 0xFF);
	...
	spf_serial_printf("\r\n");
	spf_serial_printf("w25 ReadPage done after %d ms\r\n",StartTime);
	timers_delay_microseconds(100);
	#endif	
	timers_delay_microseconds(1);
	w25.Lock=0;
