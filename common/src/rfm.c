...
// Buffer is circular so can just keep pointing to this and reading x bytes (read will go circular).
// spi_write_single(RFM_REG_0D_FIFO_ADDR_PTR, spi_read_single(RFM_REG_10_FIFO_RX_CURRENT_ADDR)); instead of clearing buffer all the time
...
{
  return ((uint16_t)(PACKETS_BUF_SIZE + packets_head - packets_tail)) % PACKETS_BUF_SIZE;
}
...
bool rfm_transmit_packet(rfm_packet_t packet)
{
  ...
  bool sent = false;
  TIMEOUT(RFM_TX_TIMEOUT, "RFM TX", 0, gpio_get(RFM_IO_0_PORT, RFM_IO_0), sent = true;, ;);
...
static void spi_setup(void)
{
  ...
  spi_disable(RFM_SPI);
  spi_init_master(RFM_SPI, SPI_CR1_BAUDRATE_FPCLK_DIV_4,
...
void exti4_15_isr(void)
{
  ...
  else
  {
    ...
    {
      spi_write_single(RFM_REG_0D_FIFO_ADDR_PTR, spi_read_single(RFM_REG_10_FIFO_RX_CURRENT_ADDR));

      // Read data from RFM
      spi_read_burst(RFM_REG_00_FIFO, packets_buf[packets_head].data.buffer, RFM_PACKET_LENGTH);
