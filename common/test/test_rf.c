 ...
 * @date    26/Dec/2020
 * @brief   Radio Module Testing Source File
 *
 * @defgroup testing Testing
 * @{
 *   @defgroup rf_test Radio Module Tests
 *   @brief    Test functions for radio frequency module
 *
 *   This file contains test functions for the radio frequency module,
 *   including channel scanning and packet transmission/reception.
 * @}
 ******************************************************************************
 */
...
/**
 * @brief Test state variables
 */
static uint8_t channel_number = 0; /**< Current channel number */
static uint8_t test_number = 0;    /**< Current test number */
static bool    receiver = false;   /**< Receiver mode flag */
...
/**
 * @brief Initialize radio frequency scanner
 *
 * Sets up the radio module for testing with specified power level
 * and configures interrupts based on receiver/transmitter mode
 *
 * @param r True for receiver mode, false for transmitter mode
...
/**
 * @brief Process next radio test step
 *
 * This function is called by the timer or external interrupt
 * to process the next step in the radio test sequence.
 *
 * For receivers: Gets received packet and prints info
 * For transmitters: Changes channel and sends test packet
 *
 * @return None
 */
void rfs_next(void) {
    ...
    if (receiver) {
        ...
        if (rfm_get_packet(&packet)) {
            log_printf("Received: %i Channel: %i Test: %i ",
                       sizeof(packet.data), packet.data.buffer[0],
