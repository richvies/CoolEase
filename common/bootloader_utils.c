...
/**
 * Mask for RCC_CSR defining which bits may trigger an entry into boot_info
 * mode:
 * - Any watchdog reset
 * - Any soft reset
 ...
 * - A firewall reset
 */
#define BOOTLOADER_RCC_CSR_ENTRY_MASK                                          \
...
void boot_init(void) {
    ...
    if ((boot_info->init_key != BOOT_INIT_KEY) &&
        (boot_info->init_key != BOOT_INIT_KEY2)) {
        BOOT_LOG("First Run\n");
    ...
    if (reset_get_flags() & RCC_CSR_IWDGRSTF) {
        ...
        if (mem_read_bkp_reg(BKUP_BOOT_OK) == BOOT_OK_KEY) {
            log_printf(".App\n");
            mem_eeprom_write_word_ptr(&boot_info->app_num_iwdg_reset,
                                      boot_info->app_num_iwdg_reset + 1);
        }
    ...
    // Magic value must be set every time bootloader runs succesfully, before
    // calling main app
...
bool boot_program_half_page(bool lower, uint32_t crc_expected,
    ...
    // Program half page
    else if (!mem_flash_write_half_page(FLASH_APP_ADDRESS +
                                            (page_num * FLASH_PAGE_SIZE) +
                                            (lower ? 0 : (FLASH_PAGE_SIZE / 2)),
                                        data)) {
        if (lower) {
...
bool boot_program_application(uint32_t* data, uint32_t start_address,
    ...
    /** Program flash if looks good */
        ...
        for (uint32_t address = start_address; address < start_address + len;
