...
class ProgramBlock(object):
    ...
    def pack(self):
        ...
        """
        if len(self.data) != 64:
            raise InvalidPageBlockException(
                "Block is not half-page sized and cannot be used directly as command"
            )
        return self.data
        ...
        while True:
            ...
            if line[0] != ":":
                raise InvalidFileException(
                    "Invalid character found at line " + str(self.line_number)
                )
            if len(line) < 3:
                raise InvalidFileException(
                    "Short line found at line " + str(self.line_number)
                )
            length = int(line[1:3], 16)
            if len(line) != 11 + 2 * length:
                raise InvalidFileException(
                    "Invalid line length at line " + str(self.line_number)
                )
            address = int(line[3:7], 16)
            record_type = int(line[7:9], 16)
            # checksum = int(line[-2:], 16)
            data = bytes(
                [int(line[9 + 2 * i : 9 + 2 * (i + 1)], 16) for i in range(0, length)]
            )
            if record_type == 0:
...
class ProgramPager(object):
    ...
    def __init__(self, loader: IntelHexLoader, page_power=7):
        self.page_size = pow(2, page_power)
        self.address_mask = self.page_size - 1
        self.segments = []
        current_segment = None
        for b in sorted(loader, key=lambda b: b.address):
...
class ResetCommand(device.Command):
    COMMAND = 0x00000000
...
class ProgramStartCommand(device.Command):
    COMMAND = 0x00000080
...
class BootloaderExitCommand(device.Command):
    COMMAND = 0x000000C3
...
class BootloaderAbortCommand(device.Command):
    COMMAND = 0x0000003E
...
class Bootloader(device.Device):
    MANUFACTURER = "CoolEase"
    PRODUCT = "LED hub Bootloader"
    ...
    def bootloader_command(self, cmd, wait_for_status=True):
        ...
        raise RuntimeError("Bootloader command failed")
    ...
    def program(self, page):
        if (page.address & 0x7F) != 0 or len(page.data) != 128:
            raise InvalidPageBlockException(
                "The passed block does not fit the definition of a 128-byte aligned page"
            )
